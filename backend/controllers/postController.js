...
const handleGetPostsByUsername = async (req, res) => {
    ...
    try {
        const posts = await Post.find({ username: username }).sort({ updatedAt: -1 }).exec();
        res.status(200).json({ posts: posts });
    } catch (error) {
        console.log(error);
...
module.exports = { handleGetAllPosts, handleGetPostsByUsername, handleCreatePostByUsername, handleDeletePost, handleUpdatePost}